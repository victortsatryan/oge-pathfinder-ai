 ...
 * Reads data through repositories; each block is isolated by SectionBoundary.
 */
function StudentHome() {
  const today = new Date().toISOString().slice(0, 10);
  const horizon = new Date(Date.now() + 1000 * 60 * 60 * 24 * 14).toISOString().slice(0, 10);

  const overview = useQuery(itemQuery(["hub", "overview"], () => analyticsRepo.overview()));
  const weak = useQuery(listQuery(["hub", "weak"], () => analyticsRepo.weakTopics(3)));
  const recs = useQuery(listQuery(["hub", "recs"], () => analyticsRepo.recommendations()));
  const events = useQuery(
    listQuery(["hub", "events", today, horizon], () =>
      learningPathRepo.calendarEvents({ from: today, to: horizon }),
    ),
  );

  const o = overview.data ?? EMPTY_STUDENT_OVERVIEW;
  ...
  return (
    <article className="pf-reader-wide pf-rise">
      <div className="pf-section-eyebrow">
        ...
        <span className="pf-section-eyebrow__label">
          {overview.isError ? (
            ...
            <>
              прогресс {o.avg_mastery}% · слабых тем {o.weak_topics}
      ...
      <section className="mb-16">
        ...
        <SectionBoundary name="Ближайшее занятие">
          {events.isError ? (
            <p className="text-sm" role="alert" style={{ color: "var(--pf-cinnabar)" }}>
              Не удалось загрузить ближайшие занятия.
            </p>
          ) : next ? (
            <div className="grid grid-cols-[120px,1fr,auto] gap-8 items-center">
              <div className="font-mono text-4xl" style={{ color: "var(--pf-ink)" }}>
                {next.event_date
      ...
      <section className="mb-16">
        ...
        />
        <SectionBoundary name="Слабые темы">
          {weak.isError ? (
            <p className="text-sm" role="alert" style={{ color: "var(--pf-cinnabar)" }}>
              Не удалось загрузить слабые темы.
            </p>
          ) : weakList.length === 0 ? (
            <p className="text-sm" style={{ color: "var(--pf-muted)" }}>
                ...
                <li key={z.topic_id} style={{ borderBottom: "1px solid var(--pf-line)" }}>
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[16px] font-medium">{z.topic_title}</div>
                    </div>
                    <div
                      className="font-mono text-[13px] whitespace-nowrap"
                      style={{
                        color:
                          z.mastery_score < 30 ? "var(--pf-cinnabar)" : "var(--pf-ink)",
                      }}
                    >
      ...
      <section className="mb-16">
        ...
        <SectionBoundary name="Рекомендации">
          {recs.isError ? (
            <p className="text-sm" role="alert" style={{ color: "var(--pf-cinnabar)" }}>
            ...
            <ol>
              {recList.slice(0, 4).map((s, i) => (
                <li key={`${s.topic_id}-${s.kind}`} style={{ borderBottom: "1px solid var(--pf-line)" }}>
                  <Link
                    to="/student/topics/$topicId"
                  ...
                  >
                    <div className="font-mono text-[12px]" style={{ color: "var(--pf-muted)" }}>
                    ...
                    <ArrowRight className="h-4 w-4" style={{ color: "var(--pf-muted)" }} />
                  </Link>
                </li>
              ))}
            </ol>
